...
public interface HistoricoConversaoRepository extends JpaRepository<HistoricoConversao, UUID> {
    ...
    @Modifying
    @Transactional
    @Query("update circulo_historico_do_circulo set circulo_id = null, where circulo_id = ?1 AND historico_do_circulo_id_conversao = ?2")
    Integer limparIdCirculoDoHistorico(UUID circuloId, UUID idConversao);
}